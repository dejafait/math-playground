...
**Conclusion.** There exists a_N∈(0,1/100] such that H_d(F_{a_N}) is positive definite for all 0≤d≤N, while F_{a_N} has nonreal zeros and every generic property in Lemma 55, including strict concavity of u↦log h_{a_N}(√u) on (0,∞) and every weighted adjacent-moment inequality stated there, simultaneously for all admissible factors, real λ and m≥1. The choice of a_N is allowed to depend on N; no single nonzero a is asserted to pass all degrees.
