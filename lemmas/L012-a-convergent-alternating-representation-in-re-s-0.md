...
**Mathlib.** The formalization uses the uniform summable-majorant theorem `tendstoUniformlyOn_tsum_nat`, the local-uniform-limit theorem `TendstoLocallyUniformlyOn.differentiableOn`, and `tendsto_rpow_neg_atTop` for the odd-index remainder. For Re(s)>1, `tsum_even_add_odd` separates the absolutely convergent series and `zeta_eq_tsum_one_div_nat_add_one_cpow` identifies its unsigned sum with zeta. The multiplier derivative and `riemannZeta_residue_one` give the punctured product limit. `Complex.analyticAt_of_differentiable_on_punctured_nhds_of_continuousAt` then proves holomorphicity of the product filled at s=1 via `Function.update`. Finally, `AnalyticOnNhd.eqOn_of_preconnected_of_eventuallyEq` extends agreement near s=2 to the right half-plane, whose preconnectedness follows from convexity. Relevant documentation:
...
**Lean proof status.** Validated on 2026-09-20 with Lean 4.33.1 and mathlib `v4.33.1`. `L012` assembles local uniform convergence of all original alternating partial sums, holomorphicity of their limit, and equality with the filled product throughout Re(s)>0. `L012_term_cpow` verifies that the exponential terms equal 1/(n+1)^s. `L012_zeta_identity` gives the ordinary product identity away from 1; `L012_eta_one` gives η(1)=log 2. The extension's holomorphicity and punctured product limit justify the removable-singularity interpretation. All fourteen printed axiom audits use only `propext`, `Classical.choice`, and `Quot.sound`; no admitted proofs or custom axioms occur. The hypotheses, conclusion, and informal proof are unchanged.

**Lean proof command.**

Validated reproduction command:

```sh
(cd scripts/lean && lake build Rh.L012)
```
