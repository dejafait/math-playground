...
**Mathlib.** This explicit counterexample is proved below by exact complex arithmetic; no prepackaged Mathlib proof of the full statement is used. The norm-square identity is `Complex.sq_norm`:

https://leanprover-community.github.io/mathlib4_docs/Mathlib/Analysis/Complex/Norm.html#Complex.sq_norm

**Lean proof status.** Validated. Lean 4.33.1, mathlib `v4.33.1`, `lake build Rh.L034` from `scripts/lean`. Kernel axioms: `propext`, `Classical.choice`, `Quot.sound`.

**Lean proof command.**

```
(cd scripts/lean && lake exe cache get && lake build Rh.L034)
```
