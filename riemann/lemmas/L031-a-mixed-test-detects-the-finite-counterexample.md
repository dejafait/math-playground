...
**Mathlib.** The explicit counterexample is proved above by exact rational arithmetic and a power-norm estimate; no prepackaged Mathlib proof of the full statement is used. Positive semidefiniteness is Mathlib's `Matrix.PosSemidef`, and its quadratic-form consequence is `Matrix.PosSemidef.dotProduct_mulVec_nonneg`:
