...
This is a mathematical obligation inventory, not a validation ledger. Lean work is paused under GOAL.md until explicitly resumed by the user; actual coverage and validation evidence live in each lemma Markdown file. PROGRESS.md is the sole current checkpoint. Unfinished or conditional formalizations are preserved for later and do not block informal research. This inventory is not an instruction to formalize now.
